...
const handleProgressUpdate = () => {
  ...
  const shouldOpen = timestampArray.filter((timestamp) => video.currentTime >= timestamp.timestamp);
  const shouldClose = timestampArray.filter((timestamp) => video.currentTime < timestamp.timestamp);
  console.log(video.currentTime);
  // console.log()
  shouldOpen.forEach((timestamp) => {
    timestamp.detailsElement.open = true;
  });
